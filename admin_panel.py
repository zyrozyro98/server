...
class AdminPanel:
    ...
    def setup_stats_tab(self, parent):
        ...
        self.stats_cards = {
            'total_licenses': self.create_stat_card(cards_frame, "إجمالي التراخيص", "0", "#3498db"),
            'active_licenses': self.create_stat_card(cards_frame, "التراخيص النشطة", "0", "#27ae60"),
            'expired_licenses': self.create_stat_card(cards_frame, "المنتهية", "0", "#e74c3c"),
            'total_devices': self.create_stat_card(cards_frame, "الأجهزة المسجلة", "0", "#9b59b6")
        }
    ...
    def create_stat_card(self, parent, title, value, color):
        ...
        return {'frame': card, 'label': value_label}
    ...
    def update_statistics(self, licenses):
        ...
        # تحديث البطاقات
        self.stats_cards['total_licenses']['label'].config(text=str(total))
        self.stats_cards['active_licenses']['label'].config(text=str(active))
        self.stats_cards['expired_licenses']['label'].config(text=str(expired))
        self.stats_cards['total_devices']['label'].config(text=str(total_devices))
